	...
	work: [
		{
			...
			endYear: null,
			expertise: {
				cto: "AI-native SaaS strategy, AWS serverless, agentic LLM extraction pipelines, supply-chain/manufacturing domain, EDI/ERP integration",
			...
			highlights: {
				cto: [
					...
					"Architected an agentic LLM document-extraction pipeline (Gemini) with Zod-typed schemas and an immutable audit trail separating raw model output from human-reviewed values.",
					"Built the platform audit-ready for regulated buyers: immutable audit trail, per-tenant row-level security, and per-Lambda IAM scoping with no shared service accounts across tenants.",
					"Shipped production integrations with 2 ERPs (Amtech DAT over SFTP, QuickBooks) and outbound X12 850 EDI, replacing manual data entry for the pilot customer.",
				...
				ic: [
					...
					"Agentic LLM document-extraction pipeline (Gemini) with Zod-typed schemas; immutable audit trail separating raw model output from human-reviewed values.",
					...
					"Forward-only Postgres migrations run through a proper SDLC with queued migration promotion: promoting a build to staging or prod applies its migrations automatically, with linting in CI to prevent drift.",
					...
					"Production integrations: Amtech DAT (SFTP), QuickBooks, outbound X12 850 EDI; Stripe billing, Supabase Auth with Google OAuth, PostHog analytics, SES-based email intake.",
					"Turborepo / pnpm monorepo spanning web, backend, and shared packages with LLM-optimized single-concern file markers.",
				],
			},
		},
		{
		...
		},
		{
			...
			endYear: null,
			expertise: {
				cto: "Independent consulting, full-stack SaaS delivery, domain-driven design, LLM-driven extraction",
				ic: "Next.js, React, TypeScript, Postgres, Python, Docker, Fly.io, Linux, SQL, SDLC, Git, LLM extraction pipelines",
			},
			highlights: {
				cto: [
					"Founder of a profitable freelancing LLC (since 2009), run alongside W-2 leadership roles.",
					"Recent: solo-delivered a multi-tenant HVAC load-calculation SaaS for an engineering consultancy from RFP to production under fixed-price contract — architected an IP boundary so the client's proprietary calc engine plugs in behind a stable interface, and shipped everything around it (data capture, AHRI equipment ranking, ASHRAE weather ingestion, PDF design reports, auth, immutable audit trail) on Next.js 16 / Postgres, deployed to Fly.io.",
				],
				ic: [
					"Founder of a long-running independent consultancy (profitable when actively engaged); broad engagements across web, mobile, data, and DevOps.",
					"Solo-delivered a multi-tenant HVAC load-calculation SaaS (Next.js 16 / React 19 / TypeScript, Postgres) from RFP to production for an engineering consultancy, replacing an Excel/Bluebeam workflow with an auditable web app.",
					"Architected an explicit IP boundary (client's proprietary calc engine behind a validated Python interface), a typed API client statically verified against server route contracts, an AHRI-catalog ETL into Postgres with SEER-tiered equipment ranking, and an immutable audit trail tying every result to its inputs and calc-engine version; deployed on Fly.io via GitHub Actions.",
					"Earlier: an LLM-driven image-to-structured-data pipeline for menu ingestion (picture → structured records).",
				],
			},
		},
		{
